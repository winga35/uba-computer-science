...
testsEj2 =
  test
    [ vacio <+> vacio ~?= vacio,
      ...
      (linea <+> linea) <+> texto "ab" ~?= linea <+> (linea <+> texto "ab"),
      mostrar( (texto "Hola mundo") <+> linea <+> (texto "Estoy vivo!")) ~?= "Hola mundo\nEstoy vivo!"
    ]
...
testsEj3 =
  test
    [ indentar 2 vacio ~?= vacio,
      ...
      mostrar( indentar 2 ((texto "Hola mundo") <+> linea <+> (texto "Estoy vivo!"))) ~?= "Hola mundo\n  Estoy vivo!"
...
testsEj6 =
  test
    [ pponObjetoSimple pericles ~?= True,
      ...
      pponObjetoSimple (TextoPP "ab") ~?= False
    ]
...
testsEj7 =
  test
    [ mostrar (intercalar (texto ", ") []) ~?= "",
      ...
      mostrar (entreLlaves [texto "A", vacio, texto "B"]) ~?= "{\n  A,\n  ,\n  B\n}"
    ]

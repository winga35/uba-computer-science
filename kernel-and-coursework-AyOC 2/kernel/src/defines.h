...
/* Indices en la gdt */
/* -------------------------------------------------------------------------- */
#define GDT_COUNT         35      //PORQUEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE
...
#define GDT_OFF_VIDEO  (GDT_IDX_VIDEO << 3) //|GDT_IDX_VIDEO|0|00|
 ...
 * Hint: usar operadores "<<" y "|" (shift y or) */
 ...
 /*
Segment selector ESTRUCTURA:
15-3 Index
2 TI 0 para GDT y 1 para LDT, lo dejamos en 0 para GDT 
1-0 RPL privilegio
 */
#define GDT_CODE_0_SEL (GDT_IDX_CODE_0 << 3) //|GDT_IDX_CODE_0|0|00| 
#define GDT_DATA_0_SEL (GDT_IDX_DATA_0 << 3) //|GDT_IDX_DATA_0|0|00|
#define GDT_CODE_3_SEL ((3)|(GDT_IDX_CODE_3 << 3)) //|GDT_IDX_CODE_3|0|11|
#define GDT_DATA_3_SEL ((3)|(GDT_IDX_DATA_3 << 3)) //|GDT_IDX_DATA_3|0|11|
// preguntar si se usananananannananananan
...
#define VIDEO_SEGM_SIZE  (COLS * ROWS * 2) 
...
// X = dir(10bits)|table(10bits)|offset(12bits)
#define VIRT_PAGE_OFFSET(X) (X&0xFFF)        //dir(10bits)|table(10bits)|offset(12bits) -> aca obtengo el indice offset
#define VIRT_PAGE_TABLE(X)  (X >> 12)&0X3FF  //dir(10bits)|table(10bits)|offset(12bits) -> aca obtengo el indice table (el indice de la tabla)
#define VIRT_PAGE_DIR(X)    (X >> 22)&0X3FF  //dir(10bits)|table(10bits)|offset(12bits) -> aca obtengo el indice dir (el indice del directorio)
#define CR3_TO_PAGE_DIR(X)  (X & 0xFFFFF000) //-> aca obtengo la dirección base del directorio
#define MMU_ENTRY_PADDR(X)  (X << 12) // 31            12 11        0 
                                      // address to page | atributos      -> Esta es la pinta de una entrada del directorio o de la tabla.
                                      // Con esta macro quiero construir la dirección base de la tabla o de la página fisica. X son los 20 bits más altos de la PDE o PTE.
                                      // Por ende necesito shiftearlos a la parte alta. 

#define MMU_P (1 << 0) //bit de presente 
#define MMU_W (1 << 1) //bit de lectura y escritura
#define MMU_U (1 << 2) //bit de usuario
